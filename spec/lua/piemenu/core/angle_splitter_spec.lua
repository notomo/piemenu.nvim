...
describe("piemenu.core.angle_splitter", function()
  ...
  for _, c in ipairs({
    ...
    },
    {
      start_angle = 0,
      end_angle = 360,
      angle_ranges = { { 120, 420 } },
      all_count = 2,
      expected = { 120, 120 + 300 / 2 },
      270,
    },
  }) do
    it(
      ("AngleSplitter.new(%s, %s, %s, %s):split() == %s"):format(
        c.start_angle,
        c.end_angle,
        vim.inspect(c.angle_ranges),
        c.all_count,
        vim.inspect(c.expected)
      ),
      function()
        local angle_ranges = require("piemenu.core.angle_range").AngleRanges.from_raw(c.angle_ranges)
        local actual = require("piemenu.core.angle_splitter").AngleSplitter.new(
          c.start_angle,
          c.end_angle,
          angle_ranges,
          c.all_count
